...
public class GeneratorGui extends PApplet implements GuiCallbackAction {
	...
	public void setup() {
		...
		selectedVisualList = cp5.addRadioButton(GuiElement.CURRENT_VISUAL.guiText(), getVisualCenter(), p5GuiYOffset-58);
		...
		cp5.addTextlabel("nfoInternalBuffer", messages.getString("GeneratorGui.INFO_INTERNAL_BUFFERSIZE")
				+md.getBufferXSize()+"/"+md.getBufferYSize(), nfoXPos, nfoYPos).moveTo(infoTab).getValueLabel(); //$NON-NLS-1$ //$NON-NLS-2$
		nfoYPos+=yposAdd;
	...
	public void draw() {
		...
		if (listener.isInternalVisualVisible()) {
			...
			for (int ofs=0; ofs<pixConServer.getNrOfVisuals(); ofs++) {
				...
				buffer = VisualState.getInstance().getAllVisuals().get(ofs).getBuffer();
				//.resizeBufferForDevice(v.getBuffer(), ResizeName.PIXEL_RESIZE, singleVisualXSize, singleVisualYSize);
				buffer = resize.resizeImage(buffer, matrixData.getBufferXSize(), matrixData.getBufferYSize(),
						singleVisualXSize, singleVisualYSize);

*/
				buffer = resize.resizeImage(pixConServer.getVisualBuffer(ofs), matrixData.getBufferXSize(), matrixData.getBufferYSize(),
						singleVisualXSize, singleVisualYSize);
		...
		if (frames%10==1) {
			...
			int snd1000 = (int)(1000f*pixConServer.getSoundImplementation().getVolumeNormalized());

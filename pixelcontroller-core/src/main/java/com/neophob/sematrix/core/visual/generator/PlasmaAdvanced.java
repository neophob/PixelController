...
public class PlasmaAdvanced extends Generator {
    ...
    public void update(int amount) {
        ...
        for (int y = 0; y < this.internalBufferYSize; y++) {
            ...
            for (int x = 0; x < this.internalBufferXSize; x++) {
                ...
                this.internalBuffer[y * internalBufferXSize + x] = swing(swing(x + swingT)
                        + swingYT)
    ...
    void makeSwingCurve() {
        ...
        for (int i = 0; i < SWINGLEN; i++) {
            float ni = i * TWO_PI / SWINGLEN; // ni goes [0..TWO_PI] -> one
                                              // complete cos wave
            swingCurveTmp[i] = (int) (Math.cos(ni * factor1) * Math.cos(ni * factor2)
                    * Math.cos(ni * factor3) * halfmax + halfmax);
        }

...
uint8 framebuffer[ELEMENT_LEDS * 3] = { 0 };

HardwareSPI led_spi(2); // SPI2 for output
...
void Element_GFX::drawPixelNum(int16 num, uint32 color)
{
  int idx;
  idx = 3 * num;
...
void Element_GFX::drawPixel(int16 x, int16 y, uint32 color)
{
...
uint32 hsv_to_rgb(uint16 hue, uint8 sat, uint8 value) // hue is [0 .. 767]
{
  uint8 temp[5], n = (hue >> 8);// % 3; // mod 3 only necessary if input goes higher than 767
  temp[0] = temp[3] = (uint8)((                                       (sat ^ 255)  * value) / 255);
  temp[1] = temp[4] = (uint8)((((( (hue & 255)        * sat) / 255) + (sat ^ 255)) * value) / 255);
  temp[2] =           (uint8)(((((((hue & 255) ^ 255) * sat) / 255) + (sat ^ 255)) * value) / 255);
